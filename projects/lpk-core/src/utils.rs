...
/// 生成解密密钥
pub fn genkey(s: &str) -> i64 {
    let mut ret: i64 = 0;
    for c in s.chars() {
        ret = (ret * 31 + c as i64) & 0xffffffff;
    }
    if ret & 0x80000000 != 0 {
...
/// 解密数据
pub fn decrypt(key: i64, data: &[u8]) -> Vec<u8> {
    ...
    for chunk in data.chunks(1024) {
        ...
        for &byte in chunk {
            ...
            ret.push((tmpkey & 0xff) as u8 ^ byte);
        }
    }
...
pub fn is_encrypted_file(s: &str) -> bool {
    ...
    let hex_part = if s.ends_with(".bin") || s.ends_with(".bin3") {
        &s[..s.len() - 4]
    } else {
        s
    };
